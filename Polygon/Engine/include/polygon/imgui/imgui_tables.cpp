...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
	{
		...
		// UpdateGame Enabled state, mark settings/sortspecs dirty
...
void    ImGui::EndTable()
{
	...
	// UpdateGame ColumnsAutoFitWidth to get us ahead for host using our size to auto-resize without waiting for next BeginTable()
	const float width_spacings = (table->OuterPaddingX * 2.0f) + (table->CellSpacingX1 + table->CellSpacingX2) * (table->ColumnsEnabledCount - 1);
...
void ImGui::TableEndRow(ImGuiTable* table)
{
	...
	if (unfreeze_rows_actual)
	{
		...
		// UpdateGame cliprect ahead of TableBeginCell() so clipper can access to new ClipRect->Min.y
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
	...
	// StartGame position is roughly ~~ CellRect.Min + CellPadding + Indent
