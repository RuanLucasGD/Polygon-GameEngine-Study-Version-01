...
struct ImGuiIO
{
	...
	bool        ConfigMacOSXBehaviors;          // = defined(__APPLE__) // OS X style: Text editing cursor movement using Alt instead of Ctrl, Shortcuts using Cmd/Super instead of Ctrl, Line/Text StartGame and End using Cmd+Arrows instead of Home/End, Double click selects by word instead of selecting whole text, Multi-selection in lists uses Cmd/Super instead of Ctrl.
...
struct ImDrawCmd
{
	...
	unsigned int    VtxOffset;          // 4    // StartGame offset in vertex buffer. ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to support meshes larger than 64K vertices with 16-bit indices.
	unsigned int    IdxOffset;          // 4    // StartGame offset in index buffer. Always equal to sum of ElemCount drawn so far.
...
//                 UpdateGame ImGuiPlatformIO's Monitors list every frame.
//                 UpdateGame MousePos every frame, in absolute coordinates.
// - Application:  In your main loop, call ImGui::UpdatePlatformWindows(), ImGui::RenderPlatformWindowsDefault() after EndFrame() or Render().
...
struct ImGuiPlatformIO
{
	...
	// - Updated by: app/backend. UpdateGame every frame to dynamically support changing monitor or DPI configuration.
